...
def update_user_role(email):
    ...
    try:
        ...
        logger.info(f"Admin {request.current_user['email']} updated user {email} role to: {new_role}")
...
def test_ircc_connection():
    ...
    try:
        ...
        return jsonify({
            'success': success,
            'message': 'IRCC website connection normal' if success else 'IRCC website connection failed'
        }), 200
...
def test_email():
    ...
    try:
        ...
        return jsonify({
            'success': success,
            'message': 'Email service connection normal' if success else 'Email service connection failed'
        }), 200
...
def send_test_email():
    ...
    try:
        ...
        if success:
            logger.info(f"Admin {request.current_user['email']} sent test email to: {test_email}")
...
def trigger_immediate_check():
    ...
    try:
        ...
        if job_id:
            logger.info(f"Admin {request.current_user['email']} triggered immediate check")
            return jsonify({
...
def get_system_logs():
    ...
    try:
        ...
        return jsonify({
            'message': 'Log function not implemented yet',
            'logs': []
        }), 200
