...
def upload_credential():
    ...
    try:
        ...
        notification_email = data.get(
            "notification_email", request.current_user["email"]
        )
...
def get_my_credentials():
    ...
    try:
        ...
        return (
            jsonify({"credentials": credential_list, "total": len(credential_list)}),
            200,
        )
        ...
        return (
            jsonify({"error": "Failed to get credentials, please try again later"}),
            500,
        )
...
def get_credential(credential_id):
    ...
    try:
        ...
        return (
            jsonify({"error": "Failed to get credential, please try again later"}),
            500,
        )
...
def update_credential(credential_id: str):
    ...
    try:
        ...
        if ircc_password:
            ...
            credential.encrypted_password = encryption_manager.encrypt(
                salt, ircc_password
            )
            if not IRCCAgentFactory.get_ircc_agent(
                credential.application_type
            ).verify_ircc_credentials(
                request.current_user["email"], credential.ircc_username, ircc_password
            ):
                return jsonify({"error": "Invalid IRCC credentials"}), 400
        ...
        logger.info(
            "User %s updated IRCC credentials successfully: %s",
...
def delete_credential(ircc_username: str):
    ...
    try:
        ...
        logger.info(
            "User %s deleted IRCC credentials successfully: %s",
            request.current_user["email"],
            ircc_username,
        )
