...
from routes.configs import config_bp
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("ircc_tracker.log"), logging.StreamHandler()],
)
...
def create_app():
    ...
    if Config.DEBUG:
        CORS(
            ...
            expose_headers=[
                "Authorization"
            ],  # Allow frontend to access Authorization header
    ...
    def serve(path):
        if path.startswith("api/") and not any(
            rule.endpoint != "static" and rule.rule.startswith("/api/")
...
def create_admin_user():
    ...
    try:
        ...
        if not existing_admin:
            # Create admin user
            admin_user = User(email=admin_email, password=admin_password, role="admin")
            admin_user.save()
